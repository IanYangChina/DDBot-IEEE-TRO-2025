...
from doma.engine.configs.macros import SAND, DTYPE_TI, DTYPE_NP
...
def main(args):
    ...
    for p in range(5):
        ...
        emd_losses = [np.zeros(200) for _ in range(6)]
        hm_losses = [np.zeros(200) for _ in range(6)]
        for i in range(200):
            ti.reset()
            ...
            for res in [10, 20, 30, 40, 50, 60]:
                ...
                emd_losses[ll][i] = emd_loss_ti[None]
                hm_losses[ll][i] = height_map_loss[None]
        ...
        for resolution in [10, 20, 30, 40, 50, 60]:
            np.save(os.path.join(result_path, f'emd_losses-res{resolution}-skill-{p}.npy'), emd_losses[ll])
            np.save(os.path.join(result_path, f'hm_losses-res{resolution}-skill-{p}.npy'), hm_losses[ll])
            ll += 1
