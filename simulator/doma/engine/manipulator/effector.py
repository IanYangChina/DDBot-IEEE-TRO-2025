...
class Effector:
    ...
    def __init__(
        ...
        self.cur_pos = ti.Vector.field(3, DTYPE_TI, needs_grad=False, shape=())  # current position of the effector
        self.quat = ti.Vector.field(4, DTYPE_TI, needs_grad=True)  # quaternion for storing rotation
        self.cur_quat = ti.Vector.field(4, DTYPE_TI, needs_grad=False, shape=())  # current quaternion of the effector
    ...
    def move(self, f):
        self.move_kernel(f)
        self.cur_pos[None] = self.pos[f + 1]
        self.cur_quat[None] = self.quat[f + 1]
    ...
    def set_state_kernel(self, f: ti.i32, controller: ti.types.ndarray()):
        for j in ti.static(range(3)):
            ...
            self.cur_pos[None][j] = controller[j]
        for j in ti.static(range(4)):
            self.quat[f][j] = controller[j + self.dim]
            self.cur_quat[None][j] = controller[j + self.dim]
