"""
Adapted from https://github.com/hzaskywalker/PlasticineLab/tree/main/plb/engine/renderer
"""
import taichi as ti
import numpy as np
import time
from doma.engine.renderer.renderer_utils import ray_aabb_intersection, inf, out_dir
...
class Renderer:
    def __init__(self, cfg=None, primitives=(), **kwargs):
        ...
        self.ground_color_ti = ti.Vector(cfg.ground_color)

        self.use_vignette = cfg.use_vignette
        self.vignette_strength = cfg.vignette_strength
        self.vignette_radius = 0.0
        self.vignette_center = cfg.vignette_center

        self.aspect_ratio = self.image_res[0] / self.image_res[1]
        self.inv_dx = 1 / self.dx

        self.camera_pos = np.asarray([float(i) for i in cfg.camera_pos])
        self.camera_rot = cfg.camera_rot
        self.camera_mat_np = np.array([
        ...
        self.light_color = cfg.light_color
    ...
    def ground_color(self, p):
        color = self.ground_color_ti * 1.0
        if p[0] <= 0.4 and p[0] >= 0.1 and p[2] <= 0.4 and p[2] >= 0.1:
            # color *= ((ti.cast(p[0] / 0.05, ti.int32) + ti.cast(p[2] / 0.05, ti.int32)) % 2) * 0.2 + 0.35
            color *= 0.99
        else:
            color *= 0.2
        return color
    ...
    def next_hit(self, o, d):
        ...
        if self.n_primitives > 0:
            if self.visualize_primitive[None]:
                ...
                while j < 200 and dist < dist_limit and sdf_val > 1e-8:
                    pp_ = o + dist * d  # ray_detection_point
                    pp = ti.Vector([pp_[0], pp_[2], pp_[1]])  # xzy -> xyz

                    sdf_val = inf
                    for i in ti.static(range(len(self.primitives))):
                        dd = ti.cast(self.primitives[i].sdf_cur_pos(pp), ti.f32)
                        if dd < sdf_val:
                            sdf_val = dd
                            sdf_id = i
                ...
                if dist < closest and dist < dist_limit and hit_primitive == 1:
                    closest = dist
                    for i in ti.static(range(len(self.primitives))):
                        if sdf_id == i:
                            n_pp_ = o + dist * d
                            n_pp = ti.Vector([n_pp_[0], n_pp_[2], n_pp_[1]])  # xzy -> xyz
                            normal_ = ti.cast(self.primitives[i].normal_cur_pos(n_pp), ti.f32)
                            normal = ti.Vector([normal_[0], normal_[2], normal_[1]])  # xyz -> xzy
        ...
        return closest, normal, color, roughness, material, hit_primitive
    ...
    def trace(self, pos, d):
        ...
        while depth < self.max_ray_depth:
            closest, normal, c, roughness, material, hit_primitive = self.next_hit(pos, d)
            ...
            if normal.norm() != 0:
                ...
                pos = hit_pos + 0.001 * d  # shadow ray origin
                throughput *= c
                ...
                if ti.static(self.use_directional_light):
                    ...
                    if dot > 0:
                        dist, _, _, roughness, shadow_hit_material, shadow_hit_primitive = self.next_hit(pos, direct)
                        if dist > dist_limit:
                            contrib += throughput * ti.Vector(self.light_color) * dot
            else:  # hit sky
                hit_sky = 1
        ...
        for i, j in self.color_buffer:
            ...
            for c in ti.static(range(3)):
                img[i, j, c] = ti.sqrt(self.color_buffer[i, j][c] * darken *
                                       self.exposure / samples)

    @ti.kernel
    def render(self, cam_pos: ti.types.ndarray(), cam_mat_up: ti.types.ndarray()):
        ti.loop_config(block_dim=128)
        # print(self.sample_sdf(self.bbox[0] + 0.05))
        # return
        mat = ti.Matrix([[cam_mat_up[0, 0], cam_mat_up[0, 1], cam_mat_up[0, 2]],
                         [cam_mat_up[1, 0], cam_mat_up[1, 1], cam_mat_up[1, 2]],
                         [cam_mat_up[2, 0], cam_mat_up[2, 1], cam_mat_up[2, 2]]])
        pos = ti.Vector([cam_pos[0], cam_pos[1], cam_pos[2]])
    ...
    def render_frame(self, spp=None, **kwargs):
        ...
        cam_pos = kwargs.get('cam_pos')
        cam_mat_up = kwargs.get('cam_mat_up')
        if cam_pos is None:
            cam_pos = self.camera_pos
            cam_mat_up = self.camera_mat_np
        else:
            cam_pos = np.asarray(cam_pos)
            cam_mat_up = np.asarray(cam_mat_up)

        for i in range(1, 1 + spp):
            # Opacity=50%
            self.visualize_target[None] = int(i % 2 == 0) * visualize_target
            self.render(cam_pos, cam_mat_up)
        ...
        self.copy(img, spp)
        return img.transpose(1, 0, 2)[::-1, ::-1]  # opencv format for render..
