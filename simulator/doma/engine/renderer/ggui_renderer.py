...
class GGUIRenderer:
    def __init__(self,
                 res=(480, 480),
                 pcd_gen_res=100,
                 height_grid_size_x=0.24,
                 height_grid_size_y=0.24,
                 height_grid_xy_offset=(0.02, 0.02),
        ...
        self.height_grid_size_x = height_grid_size_x  # mm
        self.height_grid_size_y = height_grid_size_y
        self.height_grid_xy_offset = height_grid_xy_offset  # centre point of the height map
    ...
    def init_window(self, mode):
        ...
        self.canvas.set_background_color((0.2, 0.2, 0.2))
    ...
    @ti.kernel
    def find_highest_particle_ids(self):
        for n in range(self.sim.n_particles):
            u, v = self.from_xy_to_uv(self.sim.particles_render.x[n][0], self.sim.particles_render.x[n][1])
            if self.sim.particles_render.x[n][2] >= self.height_grid[u, v]:
                self.point_id[u * self.point_cloud_gen_res + v] = n
    ...
    def render_frame(self, mode='human', t=0):
        ...
        if self.sim.has_particles:
            ...
            if mode == 'point_cloud':
                ...
                self.find_highest_particle_ids()
                points = self.sim.particles_render.x.to_numpy()
                p_ids = self.point_id.to_numpy().tolist()
                point_clouds = []
                for p_id in p_ids:
                    if p_id != -1 and p_id < 1e5:
                        point_clouds.append(points[p_id])

                points = np.asarray(point_clouds)
                return points
        ...
        if mode == 'human':
            ...
            raise ValueError(f"Invalid mode: {mode}. Choose from 'human', 'rgb_array', 'depth_array', or 'point_cloud'.")
