...
class MPMSimulator:
    def __init__(self, dim, grid_scale, gravity, horizon, dt_global, n_substeps, max_substeps_global, ckpt_dest,
        ...
        self.max_substeps_local = self.n_substeps * n_substeps_local_multiplier
        ...
        # todo: consider scale grad with n_substeps
    ...
    def setup_particle_fields(self):
        ...
        particle_state = ti.types.struct(
            ...
            J=DTYPE_TI,  # J, deformation volume ratio
                ...
                if ti.static(self.has_agent):
                    if ti.static(self.agent.collide_type in ['grid', 'both']):
                        if ti.static(self.collide_type == 'rigid'):
                            ...
                            v_out = self.agent.collide_soft(f, I * self.dx, v_out, self.dt, mf)
                        else:
                            delta_x, mat_v_toi, toi = self.agent.collide_toi(f, I * self.dx, v_out, self.dt, mf)
                            v_out = mat_v_toi
    ...
    def clip_particle_F_grad(self, f: ti.i32):
        for p in range(self.n_particles):
            ...
            elif ti.static(self.grad_op == 'substep_scale'):
                self.particles.grad[f, p].F /= self.n_substeps
    ...
    def clip_particle_x_grad(self, f: ti.i32):
        for p in range(self.n_particles):
            if ti.static(self.grad_op == 'clip'):
                ...
                self.particles.grad[f, p].x /= self.n_substeps
    ...
    def clip_action_grad(self, f: ti.i32):
        if ti.static(self.grad_op == 'clip'):
            ...
            for n in ti.static(range(len(self.agent.effectors))):
                self.agent.effectors[n].action_buffer.grad[f] /= self.n_substeps
    ...
    def clip_grid_grad(self, f: ti.i32):
        for I in ti.grouped(ti.ndrange(*self.res)):
            if ti.static(self.grad_op == 'clip'):
                ...
                self.grid.grad[f, I].mass /= self.n_substeps
                self.grid.grad[f, I].v_in /= self.n_substeps
    ...
    def print_grads(self):
        ...
        trajectory_grads = self.agent.get_grad(n=self.horizon)
