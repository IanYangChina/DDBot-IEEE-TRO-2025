...
class GatheringLoss(Loss):
    ...
    def build(self, sim):
        self.dist_weight = self.weight['dist']
    ...
    def compute_dist_loss_kernel(self, s: ti.i32, f: ti.i32):
        for p in range(self.n_particles):
            if self.particle_mat[p] == self.matching_mat:
                self.dist_loss[s] += ti.abs(self.particle_x[f, p][2] - 0.25)

    @ti.kernel
    def sum_up_loss_kernel(self, s: ti.i32):
        self.step_loss[s] += self.dist_loss[s] * self.dist_weight / self.n_particles
