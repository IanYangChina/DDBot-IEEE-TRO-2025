...
class Mesh:
    ...
    def load_file(self):
        ...
        if os.path.exists(vcolor_path):
            ...
            self.color = (COLOR[self.material][0], COLOR[self.material][1], COLOR[self.material][2])
...
class Dynamic(Mesh):
    ...
    @ti.func
    def sdf_cur_pos(self, pos_world):
        # sdf value from world coordinate
        # into mesh frame
        pos_mesh = transform_ti.inv_transform_by_trans_quat_ti(pos_world, self.container.cur_pos[None], self.container.cur_quat[None])
    ...
    @ti.func
    def normal_cur_pos(self, pos_world):
        # compute normal with finite difference
        pos_mesh = transform_ti.inv_transform_by_trans_quat_ti(pos_world, self.container.cur_pos[None],
                                                               self.container.cur_quat[None])
        pos_voxels = transform_ti.transform_by_T_ti(pos_mesh, self.T_mesh_to_voxels[None], DTYPE_TI)
        normal_vec_voxels = self.normal_(pos_voxels)
        ...
        normal_vec_world = transform_ti.transform_by_quat_ti(normal_vec_mesh, self.container.cur_quat[None])
        normal_vec_world = transform_ti.normalize(normal_vec_world)
    ...
    def collide_soft(self, f, pos_world, mat_v, dt, friction):
        ...
        if ti.static(self.has_dynamics):
            ...
            if signed_dist <= 0 or influence > 0.7:
                collider_v = self.collider_v(f, pos_world, dt)
